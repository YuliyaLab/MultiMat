...
# library(dplyr)
# library(qvalue)
# library(gdata) # for startsWith function
# library(MultiMat)
#
# source('/Users/yuliya/yuliyaFiles/UWA/projects/MultiMat/MultiMat/R/EigenMS.R') # EigenMS normlazation
# source('/Users/yuliya/yuliyaFiles/UWA/projects/MultiMat/MultiMat/R/MBImpute.R') # Model-based imputation
# source('/Users/yuliya/yuliyaFiles/UWA/projects/MultiMat/MultiMat/R/peptideLevel_DE.R') 
# source('/Users/yuliya/yuliyaFiles/UWA/projects/MultiMat/MultiMat/R/gtest.R') # G test for presence/absence comparison
# source('/Users/yuliya/yuliyaFiles/UWA/projects/MultiMat/MultiMat/R/TwoPart_MultiMS.R') # Multi matrix analysis
...
The data used in this examaple  is a subset of a proteomics experiment where
peptide IDs (sequences) have been shuffled and protein and gene IDs were replced 
by fake 'Prot_#' name.
...
Start by loading the data and defining parameter prot.info, 2 column data frame 
...
**Figure 2. Eigetrends for raw and residual peptide intensities in Human samples.** Dots at positions 
...
each otehr, as well as, 1, 3, 4 and 6 whereas in reality samples 1-3 are from 
...
```
**Figure 5. Eigetrends for raw and residual peptide intensities in Mouse samples.** 
Dots at positions 1-6 correspond to the 6 samples. Top trend in the Normalizaed Data 
(right panel) shows a pattern representative of the differences between the two groups 
(eigen trends can be rotated around x-axis). 

The eigntrend that explains most of the variation (45%) in the Mouse data is nto representative 
of the treatment groupt differences (Figure 5). The second trend in the raw data explains only 22% of 
the total variation that resambles treatmetn group differences necesitating normalization.
variation in the data as is indicated by the percentage in the upper right corner. 
...
**Figure 6. Eigetrends for raw and normalized peptide intensities in Mouse samples with the effects of one bias trends removed.**
Dots at positions 1-6 correspond to the 6 samples. Top trend in the Normalizaed Data 
Dots at positions 1-6 correspond to the 6 samples. Top trend in the Normalizaed Data 
(right panel) shows a pattern representative of the differences between the two groups. 

The eigntrend that explains most of the variation (43%) in the normalized Mouse data is representative 
of the treatment groupt differences. The second trend in the raw data explains only 27% of 
the total variation and should be concidered as bias. 
...
**Figure 7. Eigetrends for raw and normalized peptide intensities in Mouse samples with the effects of two bias trends removed.**
Dots at positions 1-6 correspond to the 6 samples. Top trend in the Normalizaed Data 
(right panel) shows a pattern representative of the differences between the two groups. 

The eigntrend that explains most of the variation in the normalized Mouse data representative 
of the treatment groupt differences now explains 58% of variation. The second trend in the 
...
```{r results = FALSE}
set.seed(1213) 
# impute based on ProtID - position in the matrix for the Protein Identifyer 
...
In the Presence-Ansence Analysis we use only proteins that are NOT in the normalized data. 
...
We strongly belieave that Model-Based Diffential Expression Analysis is a more sencitive approach 
