...
#' @param use_cols column numbers to subset and return, no range checking no range
...
#'
#' Subdivide a data frame of protein metadata and intensities into a data frame of
#' meta data only
#'
#' @param mm data frame of metadata and intensities as a signle data frame
#' @param use_cols column numbers to subset and return, no range checking on the column
...
#'   proteomics using singular value decomposition". PMID: 19602524
#' Karpievitch et al. 2009 "A statistical framework for protein quantitation in bottom-up
#'   MS-based proteomics".  PMID: 19535538
#'
...
convert_log2 = function(mm, use_cols) {
  ...
  m_logInts[m_logInts==0] = NA  # replace 0's with NA as more appropriate for analysis
...
#' Volcano plot
#'
#' Function plots fold changes and p-values as a volcano plot. Two lines are plotted
#' for the p-value cutoff at p = PV_cutoff (solid line) and p = 0.1 (dashed line).
#' @param FC vector of fold changes
#' @param PV vctor of p-values, same lenght as FC
#' @param FC_cutoff fold change cutoff where to draw vertical cutoff lines, default = 2
#' @param PV_cutoff p-value cutoff where to draw a horisontal cutoff line, default ==.05
...
#'                          pr_ppos=2)
#' plot_volcano(DE_res$FC, DE_res$BH_P_val, FC_cutoff=1.5, PV_cutoff=.05, figtitle='Mouse DE')
#' @export
plot_volcano = function(FC, PV, FC_cutoff=2, PV_cutoff=.05, figtitle='') {
  tmp_x = FC
  tt = PV
  ppos_rep = tt == 0 # exact 0 values, replace with a small values as need to take a log
  ...
  xlab='FC', ylab='-log10(p-value)',  main=figtitle)# add label to significantly different proteins
  lines(c(-(xlimits+2),(xlimits+2)), c(-log10(PV_cutoff), -log10(PV_cutoff)), col='blue')
  # also same as
...
#'        Namaes in this vector will be displayed in the volcano plot for differentially
#'        expressed proteins for this reason short names are preferred.
#' @param FC_cutoff fold change cutoff where to draw vertical cutoff lines, default = 2
#' @param PV_cutoff p-value cutoff where to draw a horisontal cutoff line, default ==.05
#' @param figtitle title to display at the top of the figure, default = ''
#' @return NULL
...
plot_volcano_wLab = function(FC, PV, ProtID, FC_cutoff=2, PV_cutoff=.05, figtitle='') {
  ...
  ggplot() + geom_point(data=plotdata, aes(x=FC, y=log_PV), alpha=0.5, size=1) +
    ...
    ylab("-log10 p-value") +
    geom_text_repel(data=filter(plotdata, threshold==TRUE), size = 3, alpha=.8, aes(x=FC, y=log_PV, label=ProtID) ) +
    theme_classic(base_size = 8) +
    geom_hline(yintercept=-log10(PV_cutoff), col='blue', alpha=.7) +
    geom_hline(yintercept=-log10(0.1), col='blue', linetype="dashed", alpha=.7) +
    geom_vline(xintercept=-FC_cutoff, col='blue', alpha=.7) + geom_vline(xintercept=FC_cutoff, col='blue', alpha=.7)
}
...
# peptideLevel_DE = function(mm, treatment, prot.info, pr_ppos=2)
  # Imputes missing values based on information from multiple peptides within a protein
  # Input:
...
#' dataset, the sum of individual statistics is the final statistic. The significance is
#' determined via a permutation test which computed the same statistics and sums them
#' after permuting the values across treatment groups. As is outlined in Karpievitch
#' et al. 2018.
#'
#' @param mm_list list of matrices for each experiment, length = number of datasets to compare
#'                internal dataset dimentions: numpeptides x numsamples for each dataset
#' @param treat list of data frames with treatment information to compute the statistic
#'              in same order as mm_list
#' @param prot.info list of protein and peptide mapping for each matrix in mm_list,
#'              in same order as mm_list
#' @param prot_col_name column name in prot.info that contains protein identifiers that
#'              link all datasets together. Not that Protein IDs will differ across
#'              different organizms and cannot be used as the linking identifier.
#'              Function match_linker_ids() produces numeric identifyers that link all
#'              datasets together
#' @param nperm number of permutations, default = 500, this will take a while, test code
#'              with fewer permutations
#' @param setseed random number generator seed, default = 12345
#' @return data frame with the following columns
#' \describe{
#'   \item{protIDused}{Column containing the protien IDs used to link proteins across datasets}
#'   \item{FC}{Average fold change across all datasets}
#'   \item{P_val}{Permutation-based p-valu for the differences between the groups}
#'   \item{BH_P_val}{Multiple testing adjusted p-values}
#'   \item{statistic}{Statistic computed as a a sum of statistics produced for each dataset}
#'   \item{Protein Information}{all columns passed into the function for the 1st dataset
#'         in the list}
#'   \item{FCs}{Fold changes for individual datasets, these values should average to the
#'         FC above. As many columns as there are datasets being analyzed.}
#'   \item{PV}{p-values for individual datasets. As many columns as there are datasets
#'         being analyzed.}
#'   \item{BHPV}{Multiple testing adjusted p-values for individual datasets. As many
#'         columns as there are datasets being analyzed.}
#'   \item{NUMPEP}{Number of peptides presents in each protien for each dataset. As many
#'         columns as there are datasets being analyzed.}
#'}
#' @examples
#' # Load mouse dataset
...
#' imp_hs = MBimpute(hs_norm_m, grps, prot.info=hs_prot.info, pr_ppos=2, my.pi=0.05,
#'                   compute_pi=FALSE, sseed=131)
#'
...
prot_level_multi_part <- function(mm_list, treat, prot.info, prot_col_name, nperm=500, setseed=12345, dataset_suffix){
  ...
  subset_data = subset_proteins(mm_list=mms, prot.info=protinfos, prot_col_name)  # grps will not change
  # names(subset_data)
  # "sub_mm_list"  "sub_prot.info"  "sub_unique_mm_list"  "sub_unique_prot.info"  "common_list"
  ...
  pos_prot_id_col = (1:length(tt))[ttt] # should be position of the column that was passed in for ID

  ## Tstat = CalcT(data, groups)  # my test here return t-stat
  # for each dataset loop through, compute, and add up t-stat values
  tmp = peptideLevel_DE(sub_mm_list[[1]], treat[[1]], sub_prot.info[[1]], pr_ppos=pos_prot_id_col)
  tstat_all = list()
  tstat_all[[1]] = tmp

  tstat = as.double(tmp[,5]) # t_value si stored in col 5:  ProtID  FC  p-val   BH_p-val  t_value   num_peptides
  FCs = as.double(tmp[,2])
  PV = as.double(tmp[,3])
  ...
  for(ii in 2:nsets){ # for second and more datasets
    tmp = peptideLevel_DE(sub_mm_list[[ii]], treat[[ii]], sub_prot.info[[ii]], pr_ppos=pos_prot_id_col)
    tstat_all[[ii]] = tmp
    tstat = cbind(tstat, as.double(tmp[,5])) # yuliya: may need to subset here, tmp is complex var
    FCs = cbind(FCs, tmp[,2])
    PV =  cbind(PV, tmp[,3])
  ...
  # perm_labels <- mt.sample.label(treat,test="t.equalvar",fixed.seed.sampling="y",B=nperm)
  ...
  ## T.Pvalue = sapply(1:npepts, function(x) (0.5+sum(abs(PermT[x,]) >= abs(Tstat[x])))/(n.perm+1))
  ...
  for(ii in 1:nsets) {
    ...
    for(jj in 1:nperm) {
      ...
      tmp = peptideLevel_DE(sub_mm_list[[ii]], treat[[ii]][perm_pos], sub_prot.info[[ii]], pr_ppos=pos_prot_id_col)
      if(jj == 1) {
 ...
 # It seesm that p-values produced by the permitation test are [0, .5], thus standard
 # adjustments will not do very well. I will rescale the interval to the max or 1.
 # p-values look the best, according to te theoretical distribution, after such adjustment
 mmin = min(p_vals)
 mmax = max(p_vals)
 adj_PV = (p_vals - mmin) / (mmax-mmin)
 FC = rowMeans(FCs)

 # protein info is on peptide level, so convert to the protein level info, no duplication
 unik = !duplicated(sub_prot.info[[1]][,prot_col_name]) # take prot IDs from dataset 1
 ppos_u_prots = seq_along(sub_prot.info[[1]][,prot_col_name])[unik]  ## indices
 u_prot_info = sub_prot.info[[1]][ppos_u_prots,]

 res = data.frame(protIDused=PROTIDS, FC, P_val=p_vals, BH_P_val=adj_PV, statistic=sum_tstat,
...
#'
#' Presence/Absence peptide-level analysis uses all peptides for a protein as IID
#' to produce 1 p-value across multiple (2+) datasets.
#' Significance is estimated using a g-test which is suitable for two treatment groups only.
#'
#' @param mm m x n matrix of intensities, number of peptides x number of samples
#' @param treatment vector indicating the treatment group of each sample ie [1 1 1 1 2 2 2 2...]
#' @param prot.info 2+ colum data frame of peptide ID, protein ID, etc. columns
#' @param pr_ppos - column index for protein ID in prot.info. Can restrict to be #2...
#'
...
#'   \item{FC}{Approximation of the fold change computed as percent missing observations
#'        group 1 munis in percent missing observations group 2}
#'   \item{P_val}{p-value for the comparison between 2 groups (2 groups only here)}
#'   \item{BH_P_val}{Benjamini-Hochberg adjusted p-values}
#'   \item{statistic}{statistic returned by the g-test, not very useful as depends on
...
#'
#' presAbs_de = peptideLevel_PresAbsDE(presAbs_dd[[1]][[1]], grps, presAbs_dd[[2]][[1]],
#'                                     pr_ppos=2)
#' @export
peptideLevel_PresAbsDE = function(mm, treatment, prot.info, pr_ppos=2){
  #   XsqDE (degrees of freedom)   -- not used, place holder for where FC goes in abundace-based DE
  #   p-value for comparison between 2 groups (2 groups only here)
  #      as interested in pariwise differences.
  ...
  for (kk in 1:length(all.proteins)) {
    ...
    peptide = as.factor(rep(1:n.peptide, each=dim(data.frame(treatment))[1]))
    # yuliya:  keep track of prIDs here...
    curr_prot.info = curr_prot.info[kk,] # possibly a subset
    y_out[kk,5] = n.peptide # good to know how many peptides were in a given protein
      ...
      if(sum(xx) < nn & sum(xx) > 0) {
        ...
        y_out[kk,2] = res$p.value # colnames(y_out) = c('XsqDF', 'p-val', 'BH_p-val', 'g_value', 'num_peptides')
        y_out[kk,1] = res$parameter
        y_out[kk,4] = res$statistic
      } else {
        y_out[kk,2] = 2 # colnames(y_out) = c('XsqDF', 'p-val', 'BH_p-val', 'g_value', 'num_peptides')
        y_out[kk,1] = 2 # all values are OUT of RANGE of proper results, will be used to flag (and remove) all preent or all absent values
        y_out[kk,4] = 2 # more likely to have all NAs
      }
      # count # of missing values in each tretment, will vary depending on the number of peptides
  ...
  # BH adjustment - only values on interval [0 1] - we may have 2 if g.test cannot be performed
  ppos = y_out[,2] <= 1
  y_out[ppos,3] = p.adjust(y_out[ppos,2],"BH")
  y_out[!ppos,3] = 1 # these would have been 2 in Raw p-values

  # XsqDF returned from the g-test is not useful, and is the same as statistic here
  # so calculate my estimate of fold change as: (% pep missing grp1) / (% pep missing grp1)

  # HERE make a dataframe to be returned - add protein names as 1st col in a data frame
...
#' dataset and sums them up to produce the final statistic. The significance is
#' determined via a permutation test which computes the same statistics and sums them
#' after permuting the values across treatment groups, as is outlined in Karpievitch
#' et al. 2018. WHenever possible proteins should be analysed using the Model-Based
#' Differential Expression Analysis due to higher statistical power over the Presence
#' Absence analysis.
#'
#'
...
#' @param prot.info list of protein metadata for each matrix in mm_list, data.frame
#'        parallel to mm_list and treat
#' @param prot_col_name column names present in all datasets that identifies protein IDs
#'        across all datasets
#' @param nperm number of permutations
#' @param setseed random number generator seed, is used to permute the data, default 13457
#' @param dataset_suffix a list of strings that will be appended to the column names
#'         for FC, PV, BHPV and numebers of peptides
#'
#' @return a data frame with the following columns:
#' \describe{
#'   \item{protIDused}{protein metadata, peptide sequence if was passed in as one of the
#'        columns is the first peptide sequence encountered in the data for that protein}
#'   \item{FCs}{Avegares across all datasets of the approximation of the fold change
#'        computed as percent missing observations group 1 munis in percent missing
#'        observations group 2 in peptideLevel_PresAbsDE() function}
#'   \item{P_val}{p-value for the comparison between 2 groups (2 groups only here)
#'        obtained from a permutation test}
#'   \item{BH_P_val}{Benjamini-Hochberg adjusted p-values}
#'   \item{statistic}{statistic returned by the g-test and summed across all datasets,
#'        not very useful as depends on the direction of the test and can produce all 0's}
#'   \item{u_prot_info}{column containing ptoein identifiers across all datasets}
#'   \item{FCs}{Approximation of the fold change computed as percent missing observations
#'        group 1 munis in percent missing observations group 2 in peptideLevel_PresAbsDE()
#'        function}
...
#'   \item{NUMPEP}{number of peptides observed for each protein in each of the datasets}
#' }
#' @export
#' @examples
#' # Load mouse dataset
...
#' intsCols = 8:13
#' metaCols = 1:7
#' m_logInts = make_intencities(hs_peptides, intsCols)  # will reuse this variable
#' m_prot.info = make_meta(hs_peptides, metaCols)
#' m_logInts = convert_log2(m_logInts)
#' grps = as.factor(c('CG','CG','CG', 'mCG','mCG','mCG')) # 3 samples for CG and 3 for mCG
#' hs_m_ints_eig1 = eig_norm1(m=m_logInts,treatment=grps,prot.info=m_prot.info)
...
#'
#' plot_volcano(presAbs_comb$FC, presAbs_comb$BH_P_val, FC_cutoff=.5, PV_cutoff=.05,
...
prot_level_multiMat_PresAbs = function(mm_list, treat, prot.info, prot_col_name,
  ...
  # select proteins that were detected in each experiment
  # make a list of unique protein IDs for each matrix in the list mm_list
  subset_data = subset_proteins(mm_list=mm_list, prot.info=prot.info, prot_col_name)  # grps will not change
  # names(subset_data)
  # "sub_mm_list"  "sub_prot.info"  "sub_unique_mm_list"  "sub_unique_prot.info"  "common_list"
  ...
  ttt = tt == prot_col_name
  pos_prot_id_col = (1:length(tt))[ttt] # should be position of the column that was passed in for ID
  tmp = peptideLevel_PresAbsDE(sub_mm_list[[1]], treat[[1]], sub_prot.info[[1]], pr_ppos=pos_prot_id_col) # mmsCA[[1]], treatsCA[[1]], protinfos[[1]], pr_ppos=2)
  # tmp = peptideLevel_DE(sub_mm_list[[1]], treat[[1]], sub_prot.info[[1]], pr_ppos=2)
  tstat_all = list()
  tstat_all[[1]] = tmp
  tstat = as.double(tmp[,5]) # t_value si stored in col 5:  ProtID  FC  p-val   BH_p-val  t_value   num_peptides
  FCs = as.double(tmp[,2])
  ...
  # prot names will be the same, will not combine them in the loop
  for(ii in 2:nsets){ # for second and more datasets
    tmp = peptideLevel_PresAbsDE(sub_mm_list[[ii]], treat[[ii]], sub_prot.info[[ii]], pr_ppos=pos_prot_id_col) # mmsCA[[1]], treatsCA[[1]], protinfos[[1]], pr_ppos=2)
         # peptideLevel_PresAbsDE(sub_mm_list[[ii]], treat[[ii]], sub_prot.info[[ii]], pr_ppos=2)
    # tmp = peptideLevel_DE(sub_mm_list[[ii]], treat[[ii]], sub_prot.info[[ii]], pr_ppos=2)
    tstat_all[[ii]] = tmp
    tstat = cbind(tstat, as.double(tmp[,5])) # yuliya: may need to subset here, tmp is complex var
    FCs = cbind(FCs, tmp[,2])
    PV =  cbind(PV, tmp[,3])
  ...
  for(ii in 1:nsets) {
    ...
    for(jj in 1:nperm) {
      # get permuted labels for each iteration, then compute T_p
      perm_pos = sample(length(treat[[ii]]), length(treat[[ii]]) )
      tmp = peptideLevel_PresAbsDE(sub_mm_list[[ii]], treat[[ii]][perm_pos], sub_prot.info[[ii]], pr_ppos=2)#  sub_mm_list[[1]], treat[[1]], sub_prot.info[[1]], pr_ppos=2)
      # tmp = peptideLevel_DE(sub_mm_list[[ii]], treat[[ii]][perm_pos], sub_prot.info[[ii]], pr_ppos=2)
      if(jj == 1) {
        tstat_perm[[ii]] =  as.matrix(as.double(tmp[,5]))
      } else {
        tstat_perm[[ii]] = cbind(tstat_perm[[ii]], as.matrix(as.double(tmp[,5])) )
  ...
  # It seems that sometimes p-values produced by the permitation test are [0, .5],
  # thus standard adjustments may not do very well.
  # One option is to rescale raw p-values to the interval with max of 1.
  # P-values can look the best (in accordance with te theoretical distribution, after
  # such adjustment.
  # Here we return BH adjusted p-values which we do nto recommend using unless the number
  ...
  res = data.frame(protIDused=PROTIDS, FC, P_val=p_vals, BH_P_val=adj_PV, statistic=sum_tstat,
                 u_prot_info, FCs, PV, BHPV, NUMPEP)
  # column names in res are inherited from the structures that are combined into the data frame
  return(res)
}
...
#' Subset proteins
#'
#' Subset proteins into ones common to all datasets passed into the function and unique
#' to each dataset. Note: for 3+ datasets no intermediate combinations of proteins are returned, only
#' proteins common to all datasets, the rest are returned as unique to each dataset.
#'
#' @param mm_list list of matrices for each experiment, length = number of datasets to compare
#'                internal dataset dimentions: numpeptides x numsamples for each dataset
#' @param prot.info list of protein and peptide mapping for each matrix in mm_list,
...
#' @return data frame with the following columns
#' \describe{
#'   \item{sub_mm_list}{list of dataframes of intensities for each of the datasets
#'         passed in with proteins present in all datasets}
#'   \item{sub_prot.info}{list of dataframes of metadata for each of the datasets
#'         passed in with proteins present in all datasets. Same order as sub_mm_list}
#'   \item{sub_unique_mm_list}{list of dataframes of intensities not found in all
#'         datasets}
#'   \item{sub_unique_prot.info}{ist of dataframes of metadata not found in all
#'         datasets}
...
#' protinfos[[2]] = imp_hs$imp_prot.info
#'
#' subset_data = subset_proteins(mm_list=mms, prot.info=protinfos, 'MatchedID')  # grps do not change
...
subset_proteins = function(mm_list, prot.info, prot_col_name) {
  ...
  # subset each experiment matrix to the proteins that are in ALL of the datasets/experiment
  # stored in common_list; need to have unique proteins from each dataset, so not doing in the loop above
  sub_mm_list = list()
  sub_prot.info = list()
  ...
  for(ii in 1:ll) {
    ...
    # each list with proteins in common between datasets needs to be in the same order
...
#'
#' Function get_presAbs_prots() produces a subset of protein meta data and intencities
#' for multiple datasets pass in as a list. If a single dataset is passed in
#' (list of length one) it will be processed in the same way as longer lists.
#'
#' @param mm_list list of matrices of intensities for each experiment.
#'                Dimentions: numpeptides x numsamples different for each dataset.
#' @param prot.info list of protein and peptide metadata/mappings for each matrix in
#'                mm_list, data.frames "parallel" to matrices in mm_list.
#' @param protnames_norm list of protein pdentifies to be used to determine peptides
#'                that will be placed into Presence/Absence analysis category due to
#'                too many missing peptides. Taken from the return value from eig_norm2().
#' @param prot_col_name column name (string) that will be used to get ProteinIDs in the raw data matrices
#'
#' @return list of lists of length 2
#' \describe{
#'   \item{intensities}{list of intecities in the same order and of the same length as
#'         the number of datasets that were passed into the function}
#'   \item{protein metadata}{list of protein metadata in the same order and of the same length as
...
#'                               protnames_norm=protnames_norm_list, prot_col_name=2)
#' @export
get_presAbs_prots = function(mm_list, prot.info, protnames_norm, prot_col_name) {
  # function get_presAbs_prots() produces a subset of protein meta data and intencities
  # for multiple datasets pass in as a list, single dataset will be processed in the same way
  # INPUT
  #   mm_list   - list of matrices of intensities for each experiment, dimentions: numpeptides x numsamples
  #   prot.info - list of protein and peptide mappings for each matrix in mm_list, data.frame parallel to mm-list
  #   protnames_norm - list of Protein Identifies to be used to determine peptides that will be
  #                    placed into Presence/Absence analysis category due to too many missing peptides
  #   prot_col_name - column name (string) that will be used to get ProteinIDs in the raw data matrices
  # OUTPUT
  #   list of lists - position 1: list of intecities, position 2: list of protein metadata,
  #                   in the order matrices were pass in
  ll = length(mm_list)
  presAbs_ints = list()
  presAbs_prot.info = list()
  for(ii in 1:ll) {
    prots_removed_pos = prot.info[[ii]][,c(prot_col_name)] %in% protnames_norm[[ii]] # negation of these are what we want...
    print(paste('Number of peptides normalized:',sum(prots_removed_pos) ) )  # peptides kept
    print(paste('Number of peptides Pres/Abs:',sum(!prots_removed_pos) ) )   # peptides eliminated
