...
#'
#' Model-Based differential expression analysis is performed on peptide level as desribed in
...
#' \describe{
#'   \item{ProtID}{protein identification information taken from prot.info, 1 column used
#'         to identify proteins}
#'   \item{FC}{fold change}
#'   \item{p-value}{p-value for the comparison between 2 groups (2 groups only here)}
...
#' DE_res = peptideLevel_DE(imp_mm$y_imputed, grps, mm_m_ints_norm$normalized[,metaCols],
#'                          pr_ppos=2)
#' @export
...
# cat(paste("Protein: ", prot, ": ", dim(y_raw)[1], " peptides (", kk, "/", length(all.proteins), ") \n", sep="" ))
    ...
    if (n.peptide != 1){
      ...
      # res = lm(yy ~ treatment + peptide, contrasts = list(treatment="contr.sum", peptide="contr.sum") )
      res = lm(yy ~ treatment + peptide, contrasts = list(peptide="contr.sum") )# baseline grp = 1st in aphanumeric order
      res1 = summary(res) # above line only good for 2 groups?
      y_out[kk,c(1,2,4)] = coefficients(res1)[2,c(1,4,3)] # col 3 reserved for BH p-values
...
#' @param sorted_norm_m normalized intensities, possibly fewer than in mm dew to
#'        filtering out peptides with fewer than one observation per treatment group
...
plot_3_pep_trends_NOfile = function(mm, prot.info, sorted_norm_m, sorted_prot.info,
  ...
  par(mfcol=c(1,3))
  ppos = imp_prot.info[,prot_to_plot_col] == prot_to_plot   # ppos = sorted_prot.info[,2] == 'O15233'  #  Q6QHH9 Q7Z384
  tmp = imp_mm[ppos,]
  if(sum(ppos) == 1) { # only 1 row, duplicate 1 row...
    tmp = rbind(tmp,tmp)
  }
  # take a Raw data, need these for y-limits
  ppos2 = prot.info[,prot_to_plot_col] == prot_to_plot # unsorted but OK, peptides will be in different order
  tmp2 = mm[ppos2,]
  if(sum(ppos2) == 1) { # only 1 row, duplicate 1 row...
    tmp2 = rbind(tmp2,tmp2)
  }
  if(dim(tmp)[1] == 0 ) { tmp = tmp2 }  # could not normalize as all obs missing in 1+ group...
  ...
  main_title = paste(gene_name, ' (', prot_to_plot, ') Normalized & Imputed', sep='')

  # outfnames_png = paste(gene_name, '_', prot_to_plot, '_3pepTrends.png',sep='') #paste(imageoutdir, '/', alldirnames[ii], '_mC_heat_bySize.png', sep='')
  # png(outfnames_png, width = 20, height = 6.4, units = 'in', res = 400) # R cannot figue out how & when res is specifed... (?)
  par(mfcol=c(1,3))
  par(mar=c(10,3,3,3))
  matplot(t(tmp), type="l", main=main_title, xaxt = "n", ylim=myylim) # may need to transpose ylim will be dynamically set here, and kept for the other plots
  ...
  matplot(t(tmp), type="l", pch='19',  main=main_title, ylim=myylim, xlim=myxlim, xaxt = "n") # may need to transpose
  axis(1, at=1:length(mylabs), labels=mylabs, las=3)
  ...
  matplot(t(tmp2), type="l", main=main_title, ylim=myylim, xaxt = "n") # may need to transpose
  axis(1, at=1:length(mylabs), labels=mylabs, las=3)
...
plot_1prot = function(mm, prot.info, prot_to_plot, prot_to_plot_col,
  ...
  tmp = mm[ppos,]
  if(sum(ppos) == 1) { # only 1 row, duplicate 1 row to be able to sued matplot function
    tmp = rbind(tmp,tmp)
  }
  matplot(t(tmp), type="l", main=prot_to_plot, xaxt = "n", col=colors) # may need to transpose ylim will be dynamically set here, and kept for the other plots
  axis(1, at=1:length(mylabs), labels=mylabs, las=3)
...
plot_3_peptide_trends = function(mm, prot.info, sorted_norm_m, sorted_prot.info, imp_mm, imp_prot.info, prot_to_plot, gene_name, mylabs) {

  par(mfcol=c(1,3))
  ppos = imp_prot.info[,2] == prot_to_plot   # ppos = sorted_prot.info[,2] == 'O15233'  #  Q6QHH9 Q7Z384
  tmp = imp_mm[ppos,]
  if(sum(ppos) == 1) { # only 1 row, duplicate 1 row...
    tmp = rbind(tmp,tmp)
  }
  # take a Raw data, need these for y-limits
  ppos2 = prot.info[,2] == prot_to_plot # unsorted but OK, peptides will be in different order
  tmp2 = mm[ppos2,]
  if(sum(ppos2) == 1) { # only 1 row, duplicate 1 row...
    tmp2 = rbind(tmp2,tmp2)
  }
...
## function
## remove_contaminats(dd, colsCheck = c('Reverse', 'Potential.contaminant'), colsCheckSymbol=c('+', '+'))
## check for the presence of '+' sign in the specificed columns but column number
remove_contaminats_symbol = function(dd, colsCheck = c('Reverse', 'Potential.contaminant'), colsCheckSymbol=c('+', '+') )
{
  # get column indeces for the specified column names, eg: 'Reverse' and 'Potential.contaminant'
  # access 'dd' by the column index to check for '+'
  cols1 = colnames(dd)
  ll = length(colsCheck)
  for(ii in 1:ll) {
    ppos = cols1 == colsCheck[ii]
    colToCheck = seq(1:length(cols1))[ppos] # position of the column we need to check
    tt1 = dd[,colToCheck] == '+'
...
## remove_contaminats(dd, colsCheck = c('Reverse', 'Potential.contaminant'), colsCheckSymbol=c('+', '+'))
## check for the presence of '+' sign in the specificed columns but column number
remove_contaminats_prefix = function(dd, colsCheck = c('Leading.razor.protein', 'Leading.razor.protein'), colsCheckPrefix=c('REV_', 'CON_') )
{
  # get column indeces for the specified column names, eg: 'Reverse' and 'Potential.contaminant'
  # access 'dd' by the column index to check for matches to the prefix in the protein names
  cols1 = colnames(dd)
  ll = length(colsCheck)
  for(ii in 1:ll) {
    ppos = cols1 == colsCheck[ii]
...
# take the first one and make it the GeneID column, store original GeneID column in GeneIDLong
...
# in case of ProtID column having -2 appended to the protein name (or any other -#), eg: Q86U42-2
# take the portion prior to -# and make it the ProtID column, store original ProtID column in GeneIDLong
# If there is not -2 or other number following the protein name, the name will be unchanged
# Protein names with -2 may create problems when matching tot he Ensembl IDs - multiple organizms only!
...
# function to use after we have fitered duplicate Protein and Gene IDs based on Ens ID
# on rare occasion there exist 2 EnsIDs for the smae GeneID but for different ProtID
# or valid protein ID in one row and missing protein ID int he 2nd row. Never seem 3 rows...
remove_dup_geneIDs = function(db)
{
  ...
  # eliminate rows with no protID in the dup ones and concatenate back the matrices
  ...
  for(ii in 1:nn) {
    ...
    if(sum(ppos_protid) > 1) {
      # keep the bottm row - in our dataset we get that as Leading Razor Protein...
...
# for each GeneID take all of the unique EnsIDs (and ProtIDs?) and make into a list?
...
get_EnsIDs = function(organizm='human') {
  ...
  lookup[2,] = c('mouse', 'mmusculus_gene_ensembl', 'mgi_symbol') # to be continued...

  lookup_pos = lookup[,1] == organizm

  ensembl = useMart("ensembl", dataset=lookup[lookup_pos,2])
  gene_symbol = lookup[lookup_pos,3]
  res = getBM(attributes=c(gene_symbol, 'uniprotswissprot', 'ensembl_gene_id'), mart=ensembl)
  dim(res)  # human: 37593
  colnames(res) = c('GeneID', 'ProtID', 'EnsID')

  # remove rows with blank ProtID, will not be an option on the proteomics dataset (?)
  ppos = res$ProtID == ''
  sum(ppos) # 16509
  res = res[!ppos,]
...
# linker - 2 column matrix with  EnsIDs that match to EnsIDs in dd1 as well as the other organizm EnsIDs
...
match_ids = function(dd1, dd2) {
  ...
  match1 = rep(NA, each=ll1)  # will NA's instead of default 0's  # vector('numeric', length=ll1)
  match2 = rep(NA, each=ll2)  # vector('numeric', length=ll2)
  num_match1 = vector('numeric', length=ll1)
  match_index = 1
  dd2$index = 1:ll2 # add index such that we could track the original position of the gene when elements are removed
